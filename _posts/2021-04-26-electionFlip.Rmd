...
knitr::opts_chunk$set(fig.path = "figure/2021-04-26-electionFlip/",fig.width=18,fig.height = 12)
```
...
The probability that the result flips is `r round(probFlip*100,2)`%.  (Here's more digits if you want them:  `r probFlip`.) Not incredibly likely, but not so infrequent that it's occurrence should instigate an investigation culminating in a [39+ minute youtube video](https://www.youtube.com/watch?v=8Ko3TdPy0TU) into what is not likely.  Suffice it to say, this is nowhere near the 10 billion human second century threshhold Matt Parker discusses in that video (also below).
