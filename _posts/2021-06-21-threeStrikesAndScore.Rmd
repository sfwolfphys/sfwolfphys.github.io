...
knitr::opts_chunk$set(fig.path = "figure/2021-06-21-threeStrikesAndScore")
```
...
The average game score is: `r mean(gameScores)`.  

There are three different scores that can be achieved:

- 60 (all three strikes in a row) occurs `r sum(gameScores==60)` times. (TDS pattern)
- 40 (two strikes in a row, third strike not consecutive) occurs `r sum(gameScores==40)` times. (`DS - S` or `S - DS` pattern)
- 30 (all three strikes have at least one gutter-ball between them) occurs `r sum(gameScores==30)` times.
