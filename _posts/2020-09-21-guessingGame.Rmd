...
knitr::opts_chunk$set(fig.path = "figure/2020-09-21-guessingGame-")
getmode <- function(v) {
   uniqv <- unique(v)
...
## Results
Before the crunching begins, let's set some bounds.  The best case is that the guessed value is the middle value that my algorithm guesses.  So we should not get a result smaller than 1.  The worst case is that I have to reduce the possibilities by half until I am left with only one choice.  This is `r ceiling(log(dictSize,2))` choices.
...
This means that we should expect to take `r round(avgSearch,2)` turns.  The most common number of turns to take is `r getmode(searchLength)`.  Table of results:

```{r echo=FALSE, results='asis'}
knitr::kable(table(searchLength))
```
...
```
And the expectation value from this calculation is `r round(g.expval,2)` (rounded to two decimal places).
